...
sys.path.append('..')
from utils.StyleFormatter import SetGlobalStyle, SetObjectStyle, SetXsystForLogScale, LatLabel, kDrays, kHInelastic, kPrimary, LineAtOne, SetLegendStyle
...
axisLimits = {'pad1': {'x': [0.01, 100], 'y': [1.e-1, 1.e+8]},
              'pad2': {'x': [0.01, 100], 'y': [0, 1.1]},
              'pad3': {'x': [0, 0.5], 'y': [0, 1.1]}}
    ...
    if pad == 1:
        for ihisto, histo in enumerate(histos):
            if ihisto == 0:
                ...
                histo.Draw('histesame')
        latALICE = LatLabel('This Analyses', 0.25, 0.85, 0.05)
        latSystem = LatLabel('Run 3 MC', 0.25, 0.8, 0.05)
        latSystem = LatLabel('pp, #sqrt{#it{s}} = 13.6 TeV',
                             0.25, 0.75, 0.05)
        legend.Draw()
    if pad == 2:
